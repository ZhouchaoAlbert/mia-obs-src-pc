...
static void AddExtraModulePaths()
{
	...
	BPtr<char> config_bin = os_get_config_path_ptr("mia-obs-studio/plugins/%module%/bin");
	BPtr<char> config_data = os_get_config_path_ptr("mia-obs-studio/plugins/%module%/data");
...
{
	char logDir[512];
	if (GetConfigPath(logDir, sizeof(logDir), "mia-obs-studio/logs") <= 0)
		return nullptr;
...
void OBSBasic::SelectCourse()
{
	// zjg add 4 select course
	if (!m_MiaCourseListDlg)
	{
		char path[512] = { 0 };
		GetConfigPath(path, sizeof(path), "mia-obs-studio/mia-img");
		m_MiaCourseListDlg = new CMainWindow(QString::fromUtf8(path));
		connect(m_MiaCourseListDlg, SIGNAL(closed()), this, SLOT(on_miaSelectCourse_destroyed()));
	}
	else
	{
		m_MiaCourseListDlg->setWindowFlags(m_MiaCourseListDlg->windowFlags() | Qt::WindowStaysOnTopHint);
	}
	m_MiaCourseListDlg->show();
}
...
{
	char path[512];
	int ret = GetConfigPath(path, 512, "mia-obs-studio");
	if (ret <= 0)
		return;
...
void OBSBasic::on_miaSelectCourse_destroyed()
{
	disconnect(m_MiaCourseListDlg, SIGNAL(miaSelectCourse_destroyed()), this, SLOT(on_miaSelectCourse_destroyed()));
	delete m_MiaCourseListDlg;
	m_MiaCourseListDlg = nullptr;
}
