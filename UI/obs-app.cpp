...
static bool MakeUserDirs()
{
	...
	if (GetConfigPath(path, sizeof(path), "mia-obs-studio/profiler_data") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

#ifdef _WIN32
	if (GetConfigPath(path, sizeof(path), "mia-obs-studio/crashes") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "mia-obs-studio/updates") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
#endif

	if (GetConfigPath(path, sizeof(path), "mia-obs-studio/plugin_config") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "mia-obs-studio/mia-img") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
...
bool OBSApp::SetTheme(std::string name, std::string path)
{
	...
	if (path == "") {
		...
		string temp = "mia-obs-studio/" + name;
...
static void move_basic_to_profiles(void)
{
	...
	strcat(new_path, "/");
	strcat(new_path, "Mia");// , Str("Untitled"));
	if (os_mkdir(new_path) == MKDIR_ERROR)
		return;
...
QString OBSApp::GetMiaAppName() const
{
	return Str("Base.System.UITitle");
}

QString OBSApp::GetMiaCourseName() const
{
	return _CourseName;
}

void OBSApp::SetMiaCourseName(const QString name)
{
	_CourseName = name;
}

QString OBSApp::GetMiaLoginGuid() const
{
	return _guid;
}

QString OBSApp::GetMiaLoginUid() const
{
	return _uid;
}

QString OBSApp::GetMiaLoginToken() const
{
	return _token;
}

void OBSApp::SetMiaLogin(const QString& guid, const QString& uid, const QString& token)
{
	_guid = guid;
	_uid = uid;
	_token = token;
...
MiaObsServerConf* OBSApp::GetMiaObsServerConf()
{
	static MiaObsServerConf s_conf;
	return &s_conf;
}
...
{
	BPtr<char>       logDir(GetConfigPathPtr("mia-obs-studio/logs"));
	struct os_dirent *entry;
	os_dir_t         *dir        = os_opendir(logDir);
...
static void create_log_file(fstream &logFile)
{
	...
	dst << "mia-obs-studio/logs/" << currentLogFile.c_str();
	...
	if (logFile.is_open()) {
		delete_oldest_file("mia-obs-studio/logs");
...
#define LITERAL_SIZE(x) x, (sizeof(x) - 1)
	ostringstream dst;
	dst.write(LITERAL_SIZE("mia-obs-studio/profiler_data/"));
...
static int run_program(fstream &logFile, int argc, char *argv[])
{
	...
	try {
		...
		delete_oldest_file("mia-obs-studio/profiler_data");
...
#define CRASH_MESSAGE \
	...
	"%appdata%\\mia-obs-studio\\crashes directory)"
...
static void main_crash_handler(const char *format, va_list args, void *param)
{
	...
	delete_oldest_file("mia-obs-studio/crashes");

	string name = "mia-obs-studio/crashes/Crash ";
...
static void move_to_xdg(void)
{
	...
	if (snprintf(old_path, 512, "%s/.mia-obs-studio", home) <= 0)
		return;
	...
	if (GetConfigPath(new_path, 512, "mia-obs-studio") <= 0)
		return;
